...
class InstructNA(nn.Module):
    def __init__(self, lm=None,lm_embeds_dim=512, lm_vocab_size=512,decoder_hidden_dim=768, n_layers=6, n_heads=8,proj_dim=8,is_evo=False):
        super(InstructNA, self).__init__()
        self.is_evo=is_evo
        self.encoder = EncoderWithProj(lm, lm_embeds_dim, proj_dim=proj_dim, lm_vocab_size=lm_vocab_size,is_evo=is_evo)
    ...
    def forward(self, x,ar_encoder=False):
